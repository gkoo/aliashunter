...
module.exports = () => {
  ...
  this.doSetup = function(msg, robot) {
    ...
    this.generateWordPool(function(wordPool) {
      ...
      // Red spymaster
      robot.messageRoom(that.teams[AliasHunter.Colors.COLOR_RED].spymaster.handle, redSpymasterMsg);

      // Blue spymaster
      robot.messageRoom(that.teams[AliasHunter.Colors.COLOR_RED].spymaster.handle, blueSpymasterMsg);
    });
  };
  ...
  this.generateTeams = function(cb) {
    var redTeam = new AliasHunter.Team({ id: 1, color: AliasHunter.Colors.COLOR_RED, name: "Red Team" });
    var blueTeam = new AliasHunter.Team({ id: 2, color: AliasHunter.Colors.COLOR_BLUE, name: "Blue Team" });
    this.teams = {};
    this.teams[AliasHunter.Colors.COLOR_RED] = redTeam;
    this.teams[AliasHunter.Colors.COLOR_BLUE] = blueTeam;
  ...
  this.generateWordPool = function(cb) {
    this.wordPool = new AliasHunter.WordPool();
  ...
  this.addPlayer = function(handle) {
    if (this.state === States.STATE_PREGAME) {
      this.players[handle] = new AliasHunter.Player({
        handle: handle
      });
    } else {
  ...
  this.getColorName = function(color) {
    switch (color) {
      case AliasHunter.Colors.COLOR_BLUE:
        return "Blue";
      case AliasHunter.Colors.COLOR_RED:
        return "Red";
      case AliasHunter.Colors.COLOR_BROWN:
        return "Brown";
      case AliasHunter.Colors.COLOR_BLACK:
        return "Black";
    }
  };
