...
var Codenames = {
  ...
  Game: function() {
    ...
    this.newGame = function() {
      if (this.state !== Codenames.States.STATE_INACTIVE) {
        throw "Couldn't start a new game because there is already a game in progress.";
      }

      this.state = Codenames.States.STATE_PREGAME;
      this.players = {};
    };

    this.start = function(msg, robot) {
      if (this.state !== Codenames.States.STATE_PREGAME) {
        throw "Something went wrong, expected state " + Codenames.States.STATE_PREGAME + " but got state " + this.state;
      }
    ...
    this.doSetup = function(msg, robot) {
      ...
      this.generateWordPool(function(wordPool) {
        ...
        // Red spymaster
        robot.messageRoom(that.teams[Codenames.Colors.COLOR_RED].spymaster.handle, redSpymasterMsg);

        // Blue spymaster
        robot.messageRoom(that.teams[Codenames.Colors.COLOR_RED].spymaster.handle, blueSpymasterMsg);
      });
    };

    this.end = function() {
      this.state = Codenames.States.STATE_INACTIVE;
    };

    this.inProgress = function() {
      return this.state !== Codenames.States.STATE_INACTIVE;
    };
    ...
    this.generateTeams = function(cb) {
      var redTeam = new Codenames.Team({ id: 1, color: Codenames.Colors.COLOR_RED, name: "Red Team" });
      var blueTeam = new Codenames.Team({ id: 2, color: Codenames.Colors.COLOR_BLUE, name: "Blue Team" });
      this.teams = {};
      this.teams[Codenames.Colors.COLOR_RED] = redTeam;
      this.teams[Codenames.Colors.COLOR_BLUE] = blueTeam;
